...
func main() {
	cfg := server.Config{
		Addr:          envAddr(),
		DBPath:        envString("MONITOR_DB_PATH", server.DefaultDBPath),
		RetentionDays: envInt("MONITOR_RETENTION_DAYS", server.DefaultRetentionDays),
	}
	...
	app := server.NewApp(store, cfg, logger)
